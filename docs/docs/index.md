...
この package の公開入口は [src/index.ts](https://github.com/srymh/vite-plugin-electron/blob/main/packages/vite-plugin-electron/src/index.ts) です。ここから `electron` と各 option 型を再 export します。
...
[src/index.ts](https://github.com/srymh/vite-plugin-electron/blob/main/packages/vite-plugin-electron/src/index.ts) では次を export しています。
...
- [src/electron.ts](https://github.com/srymh/vite-plugin-electron/blob/main/packages/vite-plugin-electron/src/electron.ts): 公開入口。Vite plugin の登録と environment 設定を担当
- [src/types.ts](https://github.com/srymh/vite-plugin-electron/blob/main/packages/vite-plugin-electron/src/types.ts): 共有型と定数。公開 option 型と内部 resolved 型の基盤を担当
- [src/options.ts](https://github.com/srymh/vite-plugin-electron/blob/main/packages/vite-plugin-electron/src/options.ts): option 正規化、preload 検証、debug 解決、spawn 用 helper を担当
- [src/environment.ts](https://github.com/srymh/vite-plugin-electron/blob/main/packages/vite-plugin-electron/src/environment.ts): custom environment 定義と environment build 設定を担当
- [src/dev-state.ts](https://github.com/srymh/vite-plugin-electron/blob/main/packages/vite-plugin-electron/src/dev-state.ts): build 判定、watch event outcome、restart scheduler を担当
- [src/dev.ts](https://github.com/srymh/vite-plugin-electron/blob/main/packages/vite-plugin-electron/src/dev.ts): dev orchestration。Vite watch build と restart 実行の接着を担当
- [src/process.ts](https://github.com/srymh/vite-plugin-electron/blob/main/packages/vite-plugin-electron/src/process.ts): Electron process lifecycle。起動、停止、Windows の taskkill を担当

この分離により、複雑な制御は helper テストで担保し、副作用を持つ処理は薄い module に閉じ込めています。

より具体的には、内部の流れは次の順序です。

1. [src/electron.ts](https://github.com/srymh/vite-plugin-electron/blob/main/packages/vite-plugin-electron/src/electron.ts) が option を解決し、custom environment と build 設定を登録する
2. [src/dev.ts](https://github.com/srymh/vite-plugin-electron/blob/main/packages/vite-plugin-electron/src/dev.ts) が dev server の `listening` を待って watch session を開始する
3. [src/options.ts](https://github.com/srymh/vite-plugin-electron/blob/main/packages/vite-plugin-electron/src/options.ts)、[src/environment.ts](https://github.com/srymh/vite-plugin-electron/blob/main/packages/vite-plugin-electron/src/environment.ts)、[src/dev-state.ts](https://github.com/srymh/vite-plugin-electron/blob/main/packages/vite-plugin-electron/src/dev-state.ts) が option 解決・environment 構成・restart 判定を担う
4. [src/process.ts](https://github.com/srymh/vite-plugin-electron/blob/main/packages/vite-plugin-electron/src/process.ts) が実際の Electron process の起動と停止を行う
...
このリポジトリには [.vscode/launch.json](https://github.com/srymh/vite-plugin-electron/blob/main/.vscode/launch.json) を用意しています。
...
現在のバージョンは [package.json](https://github.com/srymh/vite-plugin-electron/blob/main/packages/vite-plugin-electron/package.json) の `version` フィールドを参照してください。
